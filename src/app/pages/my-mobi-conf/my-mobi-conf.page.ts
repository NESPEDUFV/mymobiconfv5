...
import {
  ...
  GameConfiguracoesInterface,
} from 'src/app/services/game/configuracoes.service';
...
export class MyMobiConfPage implements OnInit {
  ...
  /** @ignore */
  @ViewChildren('principais', { read: ElementRef })
  tutBotoes: QueryList<ElementRef>;
  /** @ignore */
  ...
  ionViewWillEnter() {
    ...
    this.evento.carregar(() => {
      if (this.evento.existeDados()) {
        ...
        //participante não está participando de nenhum evento
        this.existeGamification = false;
        this.botaoPerfil = false;
    ...
    if (n != 0)
      if (this.agendadasAgora[0].mostraNotif == false) this.agendadasAgora = [];
    ...
    if (this.evento.getGamificacao() == 1) {
      if (!this.config.tutorialFoiVisto(PassosTutorial.PagPrincipal)) {
        ...
        this.passos = [
          ...
          {
            el: this.tutUtils,
            msg: PassosMsgs[PassosTutorial.PagPrincipal][0],
          },
          {
            el: this.tutNotif,
            msg: PassosMsgs[PassosTutorial.PagPrincipal][1],
          },
          { el: this.tutGame, msg: PassosMsgs[PassosTutorial.PagPrincipal][2] },
        ];
        let msgs: string[] = PassosMsgs[PassosTutorial.PagPrincipal];
        this.tutBotoes.forEach((el, i) => {
          this.passos.push({ el: el, msg: msgs[i + 3] });
        });
  ...
  mostraBotaoJogo() {
    ...
    if (!this.botaoPerfil) {
      //se o botao ainda nao esta selecionado

      if (this.participaGame) {
        //perfil já carregado
        if (
          parseInt(this.evento.getID()) == this.gameParticipante.getIDEvento()
        ) {
          //evento carregado é o inscrito
          ...
          this.gameParticipante.existePerfil(
            ...
            this.evento.getID(),
            () => {
              //verifica se já existe um perfil criado no servidor
              this.criaPerfil();
      ...
      [
        ...
        },
        {
          ...
          handler: () => {
            this.mostraAlerta(
              'Game do Evento',
              'Ao participar, você terá a chance de ter seu próprio perfil, criar ou participar de um grupo de participantes, ganhar pontos, conquistas, medalha, troféus e insígnias, além de participar de um ranking e ter informações privilegiadas sobre as avaliações de atividades e as opiniões dos participantes sobre o evento!'
            );
          },
        },
        {
  ...
  irParaRanking() {
    this.router.navigate(['/ranking-page'], {
      queryParams: { tipo: 1, primeira: true },
    });
  }
  ///Gamification
  ...
  acessarUtils() {
    if (!this.ready) return;
    if (this.evento.existeDados()) this.router.navigate(['eventos-info']);
    else this.alertEvento();
  }
  ...
  destacaBotao(botId: string) {
    if (botId == 'noticiasNotif') {
      return 'botaoGameDestaque';
    }
    return '';
  }
  ...
  carregaConfigGame(okCb?: () => void, erroCb?: () => void) {
    this.gameConfig.carregar(() => {
      ...
      if (
        this.gameConfig.existeDados() &&
        this.gameConfig.getIdEvento() + '' == this.evento.getID()
      ) {
  ...
  carregarPerfilGame(novo?: boolean) {
    this.gameParticipante.carregar(() => {
      if (
        this.gameParticipante.existeDados() &&
        this.usuario.getID() == this.gameParticipante.getID() + '' &&
        this.evento.getID() == this.gameParticipante.getIDEvento() + ''
      ) {
